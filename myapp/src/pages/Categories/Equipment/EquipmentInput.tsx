...
const EquipmentInput: React.FC<EquipmentInputProps> = ({ onClose, onSuccess }) => {
  ...
  const { postData, error: saveError } = useApi(equipmentPath, { autoFetch: false }); // autoFetch: false
...
await new Promise(r => setTimeout(r, 0));
  ...
  return (
    <LayoutInput
      ...
      formRowComponent={
        <FormRow
          title="Đơn giá điện năng (kWh)"
          title1="điện năng (kWh)"
    ...
    >
      ...
      {saveError && <p className="text-red-500 mt-2">❌ Lỗi: {saveError.toString()}</p>}
    </LayoutInput>
  );
};
